...
class GmQueue extends Queue implements QueueInterface {
    ...
    /**
     * @param GmClient $client
     * @param GmWorker $worker
     * @param $queue
     */
    public function __construct(GmClient $client, GmWorker $worker, $queue)
    {
        $this->client = $client;
        $this->worker = $worker;
        $this->queue = $queue;
     ...
     * @return int|mixed Return Gm code
     */
